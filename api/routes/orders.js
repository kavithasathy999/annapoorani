  ...
  try {
    ...
    let query = `
      SELECT o.*, 
             ...
             (
               SELECT GROUP_CONCAT(CONCAT(COALESCE(ps.product_name, p.product_name, CONCAT('Product #', ps.product_id)), ' (Qty: ', ps.qty, ')') SEPARATOR ', ')
    ...
    if (search) {
      query += ' AND (o.order_no LIKE ? OR c.name LIKE ? OR c.phone_number LIKE ?)';
...
router.get('/today/stats', async (req, res) => {
  try {
    ...
    if (search) {
      baseQuery += ' AND (o.order_no LIKE ? OR c.name LIKE ? OR c.phone_number LIKE ?)';
      params.push(`%${search}%`, `%${search}%`, `%${search}%`);
    }

    const [[{ totalOrders }]] = await pool.query(`SELECT COUNT(*) as totalOrders ${baseQuery}`, params);
    const completedParams = [...params, 'Complete'];
    const [[{ totalRevenue }]] = await pool.query(`SELECT COALESCE(SUM(o.total), 0) as totalRevenue ${baseQuery} AND o.status = ?`, completedParams);
...
router.get('/:id', async (req, res) => {
  try {
    const [orders] = await pool.query(
      `
      SELECT o.*, c.name as customer_name, c.phone_number as customer_phone, c.address as customer_address,
    ...
    const [items] = await pool.query(
      ...
      SELECT
        ...
        CASE
          ...
          ELSE 0
        END AS price,
        CAST(ps.product_total AS DECIMAL(10,2)) AS total,
        COALESCE(ps.product_name, p.product_name, CONCAT('Product #', ps.product_id)) as product_name
      FROM product_slots ps
      LEFT JOIN products p ON ps.product_id = p.id
      WHERE ps.order_id = ?
    `,
